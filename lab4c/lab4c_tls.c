...
void get_temp(){
	...
	if(begin || (during_time % period == 0 && during_time > 0)){
		if(run_flag){
			...
			if(SSL_write(sslClient, outstr, strlen(outstr)) < 0){
				fprintf(stderr, "%s\n" ,strerror(errno));
				exit(2);
			}
			if(log_flag){
...
void shut_down(){
	...
	SSL_write(sslClient, outstr, strlen(outstr));
	mraa_aio_close(tmpr);
...
int main(int argc, char *argv[]){
    ...
    bzero(msg, 50);
    sprintf(msg, "ID=%s\n", id);
    SSL_write(sslClient, msg, strlen(msg));
	...
	while(1){
		...
		if(fd[0].revents & POLLIN){
			bzero(str, 128);
			bzero(buf, 28);
			int r = SSL_read(sslClient, str, 128);
			if( r <= 0){
